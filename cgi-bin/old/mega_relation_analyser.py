...
def GetLinesFromApi(page, add = "", write_name = ""):
    print "Downloading : http://www.openstreetmap.org/api/0.6/" + page + add
    sys.stdout.flush()
...
def GetNodesForWay(wayid, add):
    nodes_coords = {}
    nodes_order  = []
    for line in GetLinesFromApi("way/"+str(wayid)+"/full", add):
...
def GetWaysForRelation(relationid, add = "", cache = ""):
    ways = []
    lines = GetLinesFromApi("relation/" + str(relationid), add)
    for line in lines:
        if line.startswith("<member "):
            MembType = ReGetType.findall(line)[0][7:-1]
            MembRef  = ReGetRef.findall(line)[0][6:-1]
            if MembType == "way":
                ways.append(GetNodesForWay(MembRef, add))
            elif MembType == "relation":
                ways += GetWaysForRelation(MembRef, add)
    
    lines_name = [x for x in lines if 'k="name"' in x]
    if lines_name:
        open(cache + "relation-" + str(relationid) + ".name", "w").write(lines_name[0].split('"')[3].encode("utf8"))
    else:
        open(cache+"relation-"+str(relationid)+".name", "w")
        
    open(cache + "relation-" + str(relationid) + ".mbrs", "w").write(str(len(ways)))
...
def WaysToSvg(ways, file):
    ...
    for i in range(len(ways)):
        ...
        file.write("\" fill=\"none\" stroke=\"#0000FF\" stroke-width=\""+str(taille/200)+"\" stroke-linecap=\"round\" stroke-linejoin=\"round\" />\n")
        if ways[i][0][0] <> ways[i][-1][0]:
            d = taille/20
            x = marge+float(ways[i][0][2])-b[1]
            y = marge+b[2]-float(ways[i][0][1])
            file.write("<path d=\"M " + str(x-d) + "," + str(y-d) + " L " + str(x+d) + "," + str(y+d) + "\" fill=\"none\" stroke=\"#FF0000\" stroke-width=\""+str(taille/50)+"\" stroke-linecap=\"round\" stroke-linejoin=\"round\" />\n")
            file.write("<path d=\"M " + str(x+d) + "," + str(y-d) + " L " + str(x-d) + "," + str(y+d) + "\" fill=\"none\" stroke=\"#FF0000\" stroke-width=\""+str(taille/50)+"\" stroke-linecap=\"round\" stroke-linejoin=\"round\" />\n")
...
def MakeSvg(ways, ways2, NumRel):
    b2 = WaysToBbox(ways2)
    taille = min(b2[2]-b2[0], b2[3]-b2[1])
    marge  = taille / 20
    d = taille/20
    b = WaysToBbox(ways)
    center = [(b[0]+b[2])/2, (b[1]+b[3])/2]
    #print "centre : " + str(center[0]) + " " + str(center[1])
    file = open("../cache/relation-" + str(NumRel) + ".svg", "w")
    
    file.write("<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"no\"?>\n")
    file.write("<svg width=\""+str(abs(b2[3]-b2[1])+2*marge)+"\" height=\""+str(abs(b2[2]-b2[0])+2*marge)+"\">\n")
    
    for i in range(len(ways2)):
        file.write("<path d=\"M " + " L ".join(str(marge+float(x[2])-b2[1]) + "," + str(marge+b2[2]-float(x[1])) for x in ways2[i]))
        file.write("\" fill=\"none\" stroke=\"#0000FF\" stroke-width=\"" + str(taille/200) + "\" stroke-linecap=\"round\" stroke-linejoin=\"round\" />\n")
    
    list = [(open("../cache/relation-" + str(NumRel) + ".nod").readlines(),"#FF0000"),
            (open("../cache/relation-" + str(NumRel) + ".nod2").readlines(),"#00FF00")]

    for lines, color in list:
        for line in lines:
            nid, lat, lon = line.strip().split(" ")
            nid, lat, lon = PointToMercator((nid, float(lat), float(lon)), center)
            x = marge + float(lon) - b2[1]
            y = marge + b2[2] - float(lat)
            file.write("<path d=\"M " + str(x-d) + "," + str(y-d) + " L " + str(x+d) + "," + str(y+d) + "\" fill=\"none\" stroke=\"" + color + "\" stroke-width=\"" + str(taille/50) + "\" stroke-linecap=\"round\" stroke-linejoin=\"round\" />\n")
            file.write("<path d=\"M " + str(x+d) + "," + str(y-d) + " L " + str(x-d) + "," + str(y+d) + "\" fill=\"none\" stroke=\"" + color + "\" stroke-width=\"" + str(taille/50) + "\" stroke-linecap=\"round\" stroke-linejoin=\"round\" />\n")
...
def PointToMercator(pt, center):
    x = 100 * float(pt[2]) - center[0]
    y = 100 * (180/math.pi) * math.log(math.tan( (math.pi/4) + (math.pi/360)*float(pt[1]) ) )
    #print float(pt[1]), y
